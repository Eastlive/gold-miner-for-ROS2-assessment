...
{
MinePublisherNode::MinePublisherNode(const rclcpp::NodeOptions & options)
: Node("mine_publisher", options)
{
  init_mine_msg();
  ...
  timer_ =
    ...
    std::bind(&MinePublisherNode::timer_callback, this));
}
...
void MinePublisherNode::init_mine_msg()
{
  ...
  for (int i = 0; i < mine_num; i++) {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Mine %d Position: (%f, %f, %f)", ore.id, ore.pose.position.x, ore.pose.position.y,
      ore.pose.position.z);
